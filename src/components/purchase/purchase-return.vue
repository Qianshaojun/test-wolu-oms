                ...
                <td style="padding: 0;">
                    <table class="xtb" style="border:none;">
                        ...
                        <tr v-for="(x, i) in info" :key="i">
                            <td :key="x.default_code">{{ x.default_code }}</td>
...
export default class PurchaseReturn extends Vue {
    @Emit('modal.submit')
    private submit() {
        return true
    }
    ...
    private onSubmit() {
        ...
        this.innerAction.setActionAPI(
            'purchase_requirement/refuse_make_purchase_order',
        ...
        this.publicService
            .modify(
                new RequestParams(
                    {
                        refuse_lines: this.info.map(x => x.id),
