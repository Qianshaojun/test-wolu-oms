...
import EditDeShipRatio from '../picking/edit-de-ship-ratio.vue'
...
export default class FinalShipManage extends Vue {
    @Emit('modal.submit')
    private submit(data) {
        return data
    }
    ...
    private columns: any = [
        ...
        {
            key: 'gls_percent',
            title: this.$t('columns.gls_percent'),
        ...
        },
        {
            ...
            title: this.$t('columns.gls_theoretical_freight'),
    ...
    private updateData(params) {
        let item = this.data.find(x => x.id == params.id)
        if (item) {
            for (let i in params) {
                if (item[i] != undefined) {
                    item[i] = params[i]
    ...
    private updateCalcResult() {
        let ids: any = this.data.map(x => x.price_id)
        this.innerAction.setActionAPI(
...
<i18n>
{
    ...
    "zh-cn": {
        "columns":{
            ...
            "gls_theoretical_freight":"GLS理论运费",
            "mfn_theoretical_freight":"自发货理论运费"
        },
        "err_msg":"总比例数必须等于100"
    }
}
