<template>
    <section class="component edit-customer">
        ...
        >
            ...
            <template #action>
                <a-dropdown :trigger="['click']">
                    <a-menu slot="overlay">
                        <a-menu-item
                            @click="setProdFloatPrice"
                            :disabled="!selectedRowKeys.length"
                            >{{ $t('action.set_prod_float_price') }}
                        </a-menu-item>
                        <a-menu-item @click="excelImportProdFloatPrice"
                            >{{ $t('action.excel_import_prod_float_price') }}
                        </a-menu-item>
                        <a-menu-item @click="excelImportProdDiscount"
                            >{{ $t('action.excel_import_prod_discount') }}
                        </a-menu-item>
                        <a-menu-item
                            @click="deCancelProductFloat"
                            :disabled="!selectedRowKeys.length"
                            >{{ $t('action.de_cancel_prod_float') }}
                        </a-menu-item>
                        <a-menu-item
                            @click="ukCancelProductFloat"
                            :disabled="!selectedRowKeys.length"
                            >{{ $t('action.cancel_prod_float') }}
                        </a-menu-item>
                        <a-menu-item
                            @click="cancelProdDiscount"
                            :disabled="!selectedRowKeys.length"
                            >{{ $t('action.cancel_prod_discount') }}
                        </a-menu-item>
                        <a-menu-item @click="sendChangeEmail"
                            >{{ $t('action.send_change_email') }}
                        </a-menu-item>
                    </a-menu>
        ...
        <a-card class="margin-y autoFlex" style="margin:0 !important;">
            <div v-if="!groupbyList.length" class="price_table">
                <AutoColumnTable
                    :stripe="true"
                    :data="data"
                    :page="pageService"
                ...
                >
                    ...
                    <template slot="active" slot-scope="text">
                        ...
                        <span v-else-if="text === false">已归档</span>
                    </template>
...
<style>
.price_table .ant-table-row-level-1:nth-child(odd),
.price_table .ant-table-row-level-1:nth-child(even) {
    background: #dddddd;
}

.price_table .ant-table-row-level-1 .ant-checkbox-wrapper {
    display: none;
}
</style>
...
export default class PriceCheckResultContent extends Vue {
    // 表格组件
    @Ref()
    ...
    private queryUrl: any =
        'product_management/query_all_product_price_check_result'
        ...
        this.getQueryCondition().then(nowConditions => {
            ...
            if (this.groupbyList.length) {
                ...
                this.innerAction.setActionAPI(
                    this.queryUrl,
                    CommonService.getMenuCode(this.page_flag)
                )
                this.publicService
                    .queryPagination(
    ...
    private onTrClick(record) {
        let info = this.data.find(x => x.index == record)
        if (info) {
            this.onDetail(info)
        }
    }

    private onDetail(info) {
        this.innerAction.setActionAPI(
            'product_management/query_product_price_check_result_line',
            CommonService.getMenuCode(this.page_flag)
        )
        this.publicService
            .query(
            ...
            .subscribe(
                data => {
                    if (data.length) {
                        ...
                        let cd: any = data.filter(x => x.is_total === undefined)
                        cd.push(lastRow)
                        info['children'] = cd
    ...
    private onPreCheck() {
        ...
        this.innerAction.setActionAPI(
            ...
            CommonService.getMenuCode('product_price_check_result')
        )
        this.publicService
            .query(
                new RequestParams(
    ...
    private excelImportProdFloatPrice() {
        this.$modal
            .open(
                UploadExcel,
                {
    ...
    private excelImportProdDiscount() {
        this.$modal
            .open(
                UploadExcel,
                {
