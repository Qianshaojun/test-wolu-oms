<template>
    <section class="component edit-customer">
        ...
        >
            ...
            >
            <!-- <a-button @click="onAdd()" style="margin-left:5px;" size="small">
                {{ $t('action.add') }}
            </a-button> -->
            <a-button
                @click="onPreCheck()"
            ...
            <a-dropdown :trigger="['click']">
                <a-menu slot="overlay">
                    <a-menu-item @click="onDelete">Delete</a-menu-item>
                    ...
                    <a-menu-item @click="onFlagToUpdate('update')">{{
                        $t('action.flag_to_update')
                    }}</a-menu-item>
                    <a-menu-item @click="onFlagToUpdate('new')">{{
                        $t('action.flag_to_new')
                    }}</a-menu-item>
                    <a-menu-item @click="onFlagToUpdate('allnew')">{{
        ...
        >
            ...
            <a-tab-pane tab="历史核价" key="history">
                <a-table
                ...
                >
                    ...
                    <a-table-column
                        :title="$t('check_date')"
                        key="check_date"
                        data-index="check_date"
                        align="center"
                        width="100px"
                    ></a-table-column>
                    <a-table-column
                        :title="$t('approve_state')"
                        key="approve_state"
                        data-index="approve_state"
                        align="center"
                        width="100px"
                    ></a-table-column>
                    <a-table-column
                    ...
                    >
                        <template slot-scope="row">
                            <a @click="onHistoryView(row)">{{ $t('view') }}</a>
                        </template>
                    </a-table-column>
                </a-table>
            </a-tab-pane>
...
export default class ProductPriceCheckEdit extends Vue {
    @Emit('modal.submit')
    private submit() {
        return true
    }
    ...
    private onSubmit() {
        let fromDetail: any = this.$refs.fromDetail
        let ret = fromDetail.getFormData()
        ret['save_flag'] = 1
    ...
    private onPanelChange(e) {
        ...
        if (e === 'history') {
            this.innerAction.setActionAPI(
                'product_management/query_product_price_check_history_by_sku',
                CommonService.getMenuCode('product_price_check')
            )
            this.publicService
                .query(
    ...
    private onHistoryView(row) {
        this.innerAction.setActionAPI(
            'product_management/query_product_price_check_history_info',
            CommonService.getMenuCode('product_price_check_history')
        )
        this.publicService
            .query(
                new RequestParams(
