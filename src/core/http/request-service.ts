            ...
            .then((response: AxiosResponse<any>) => {
                ...
                if (response.headers.customer_key) {
                    localStorage.setItem(
                        'session_id',
                        response.headers.customer_key
                    )
                    const now = moment()
                    if(!localStorage.getItem('session_expires') || response.config.url == "wms/user_login"){
                        localStorage.setItem(
                            'session_expires',
                            now.add(7, 'days').format()
                        )
                    }
                    
                }
                if (
                    !RequestService.interceptors.status.defined ||
                    RequestService.interceptors.status.interceptor(response)
                ) {
                    // 通讯成功
                    return {
                        data: RequestService.interceptors.success.defined
                    ...
                    if (RequestService.interceptors.error.defined) {
                        try {
                            return_err = RequestService.interceptors.error.interceptor(
                                response
                            )
                        } catch (error) {
