...
export default class UploadExcel extends Vue {
    ...
    private handleUpload() {
        ...
        fileList.forEach(file => {
            ...
            formData.append('columns', this.columns.map(x => x.key).join(','))
            formData.append('table_name', this.table_name)
            formData.append('update_key_columns', this.update_key_columns)
            if (this.uploadParams) {
                for (let i in this.uploadParams) {
                    if (this.uploadParams[i]) {
                        formData.append(i, this.uploadParams[i])
                    }
                }
            }
            num++
                ...
                try {
                    ...
                    if (obj.code == 0) {
                        ...
                        this.$message.success(
                            '上传成功.' + JSON.stringify(obj.message || '')
                        )
                        this.submit(obj)
                    } else {
