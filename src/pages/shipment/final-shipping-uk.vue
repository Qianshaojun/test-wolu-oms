    ...
    <page-container ref="pageContainer" :showHeader="false">
        ...
        <a-card class="margin-y autoFlex" style="margin:0 !important;">
            <div v-if="!groupbyList.length">
                ...
                >
                    <template
                        slot="is_dynamic_calculation"
                        slot-scope="text, row"
                    >
                        <span v-if="row.is_dynamic_calculation">
                    ...
                    <template
                        slot="theoretical_freight_hermes"
                        slot-scope="text, row"
                    >
                        {{ getFreightHermes(row) }}
                    </template>
                    <template
                        slot="theoretical_freight_yodel"
                        slot-scope="text, row"
                    >
                        {{ getFreightYodel(row) }}
                    </template>
...
export default class FinalShippingUK extends Vue {
    // 表格组件
    @Ref()
    ...
    private getFreight(row) {
        ...
        for (let i in row) {
            if (
                i.indexOf(instance + '_uk_') == 0 &&
                i.indexOf('hermes') == -1
            ) {
                let key =
                    i.substring(instance.length + 4) + '_theoretical_freight'
...
<i18n>
{
    "en-us": {
        ...
        "action": {
            ...
            "batch_update_ship_ratio": "Batch Update Ship Ratio(UK)"
        },
        "rules": {
        ...
        "page_name": "UK_Theoretical_Freight"
    },
    "zh-cn": {
        "plzInput": "请输入",
        "desc": "这是订单页面1",
        ...
        "action": {
            ...
            "batch_update_ship_ratio": "物流比例批量更新(英仓)"
        },
        "rules": {
