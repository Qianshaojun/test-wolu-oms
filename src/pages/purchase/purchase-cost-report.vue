<template>
    <page-container ref="pageContainer" :showHeader="false">
        ...
        <data-form ref="dataForm" @submit="getPurchaseCostData" :column="2">
            <!--默认显示项-->
            <template #default>
                <a-form-item
            ...
            <data-table
                ...
                :rowSelection="{
                    ...
                    onChange: (keys, select_rows) => {
                        ...
                        this.showCostChart(select_rows)
                    }
                }"
                :columns="columnList"
                @on-page-change="getPurchaseCostData"
                @onClick="
                    row => {
        ...
        <a-card title="趋势图">
            <ve-line :data="purchaseCostCharData"></ve-line>
        </a-card>
    </page-container>
</template>
...
export default class PurchaseCostReport extends Vue {
    // 表格组件
    @Ref()
    ...
    private purchaseCostCharData: any = []
    private query_type: String = 'year'

    private showCostChart(selectedRows) {
        let row_list: any[] = []
                ...
                for (let column_index in this.columnList) {
                    ...
                    if (column_info.children) {
                        for (let children_index in column_info.children) {
                            ...
                            this.purchaseCostCharData.rows[cell_index][
                                default_code
                            ] = row[columnName]
                        ...
                        this.purchaseCostCharData.rows[column_index][
                            default_code
                        ] = row[columnName]
    ...
    private exportData() {
        this.getQueryCondition().then((x: any) => {
            CommonService.exportData(
                '/purchase_requirement/export_purchase_price_report',
