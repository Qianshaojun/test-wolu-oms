<template>
    <page-container ref="pageContainer" :showHeader="false">
            ...
            <template #default>
                ...
                >
                    <a-select
                        ...
                        :style="{ width: '200px' }"
                        placeholder="Please Select"
                        v-decorator="['user_require', { initialValue: '' }]"
                        :filterOption="filterSelectOption"
                        size="small"
                        allowClear
                    >
                ...
                >
                    ...
                    >
                        ...
                        <a-select-option value="req_name">
                            {{ $t('columns.req_name') }}
                        </a-select-option>
                    </a-select>
                    <a-input
        ...
        <a-card v-if="selectedRowKeys[0]">
            <a-tabs
                defaultActiveKey="require_log"
                @change="e => onPanelChange(e)"
            >
                <a-tab-pane :tab="$t('require_log')" key="require_log">
                    <LogView
                ...
                <a-tab-pane :tab="$t('ship_log')" key="ship_log">
                    <LogView
                        :object_name="object_name"
                        :record_code="record_code"
                        :is_special_table="true"
...
export default class PurchaseRequirementHistoryReport extends Vue {
    // 表格组件
    // 表格组件
    @Ref()
    ...
    private queryUrl: any =
        'report_management/query_all_product_purchase_histry'
    ...
    private object_name: any = 'req_line'
    ...
    private currentLogType: any = 'require_log'

    private onTrClick(record) {
        this.currentRow = this.data.find(x => x.id === record)
...
<i18n>
{
    "en-us": {
        ...
        "require_log": "Requirement Log",
        "ship_log": "Shipement Log"
    },
    "zh-cn": {
        "desc": "这是订单页面1",
        "action": {
